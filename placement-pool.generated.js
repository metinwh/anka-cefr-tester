(function () {
  ...
  function simplifyGeneratedPrompt(prompt, family) {
    ...
    if (directSentence && wordCount(directSentence[1]) <= 18) return cleanupPrompt(directSentence[1]);
    ...
    if (chooseMatch) return cleanupPrompt(chooseMatch[1]);

    const naturalMatch = text.match(/^(?:You are|You|Your|A|The|Someone).+?\b(What is the most natural [^?]+\?)$/i);
    if (naturalMatch) return cleanupPrompt(naturalMatch[1]);
  ...
  addNaturalnessFamily({
    ...
    variants: [
      { prompt: "A teammate promised a file this morning, and you need it now. You want a firm but normal chat message.", best: "Hi, just checking whether the file is ready.", acceptable: "Is the file ready?", weak: "Where is the file?", acceptableCredit: 0.7, weakCredit: 0.42, weakRole: "weak", weakLabel: "too_abrupt" },
      { prompt: "A colleague made a small mistake in a shared document. You want to point it out politely.", best: "I noticed a small issue in the second paragraph.", acceptable: "There is a mistake in the second paragraph.", weak: "You made a mistake in the second paragraph.", acceptableCredit: 0.72, weakCredit: 0.38, weakRole: "weak", weakLabel: "personal_blame" },
  ...
  addNaturalnessFamily({
    ...
    variants: [
      { prompt: "The hotel was cheap, but it was far from the center. You want to express the trade-off naturally.", best: "It was cheap, but the location wasn't great.", acceptable: "It was cheap, but far from the center.", weak: "It was cheap because it was far from the center.", acceptableCredit: 0.8, weakCredit: 0.42, weakRole: "weak", weakLabel: "wrong_relation" },
  ...
  addNaturalnessFamily({
    ...
    variants: [
      ...
      { prompt: "A study supports one explanation but not conclusively.", best: "This explanation is plausible, but not conclusive.", acceptable: "This explanation could be right, but we need more data.", weak: "This explanation is probably correct.", acceptableCredit: 0.78, weakCredit: 0.45, weakRole: "weak", weakLabel: "underhedged" },
      ...
      { prompt: "You are summarizing mixed feedback from users.", best: "The feedback is mixed, but several users raised the same concern.", acceptable: "Some people liked it and some did not.", weak: "Users have many different opinions.", acceptableCredit: 0.66, weakCredit: 0.48, weakRole: "weak", weakLabel: "too_vague" },
  ...
  addNaturalnessFamily({
    ...
    variants: [
      ...
      { prompt: "A speaker says, 'It's not ideal, but it's workable.' What is the most natural response in a team meeting?", best: "So you think it is acceptable, even if it has drawbacks.", acceptable: "So it can work, but it is not perfect.", weak: "So it is a bad idea.", acceptableCredit: 0.78, weakCredit: 0.28, weakRole: "weak", weakLabel: "misreads_stance" },
      ...
      { prompt: "Your manager says, 'Let's keep this in mind, but not act on it yet.' Choose the best interpretation.", best: "We should remember it, but no decision is needed now.", acceptable: "We should think about it later.", weak: "We should start it immediately.", acceptableCredit: 0.74, weakCredit: 0.12, weakRole: "weak", weakLabel: "wrong_action" },
      { prompt: "A colleague says, 'I wouldn't call it a failure, but it didn't meet the goal.' Choose the best summary.", best: "The result was disappointing, though not a complete failure.", acceptable: "It was not a full failure, but it was not successful.", weak: "It was successful.", acceptableCredit: 0.78, weakCredit: 0.1, weakRole: "weak", weakLabel: "wrong_stance" },
      { prompt: "A reviewer writes, 'The argument is interesting, but the evidence is thin.' Choose the best revision note.", best: "Keep the argument, but strengthen the evidence.", acceptable: "Add more evidence to support the argument.", weak: "Change the whole topic.", acceptableCredit: 0.82, weakCredit: 0.24, weakRole: "weak", weakLabel: "overreacts" },
      ...
      { prompt: "A teacher says, 'Your answer is on the right track, but it needs more detail.' Choose the best next step.", best: "Keep the main idea and add more support.", acceptable: "Add more details to the answer.", weak: "Delete the answer and start again.", acceptableCredit: 0.8, weakCredit: 0.22, weakRole: "weak", weakLabel: "overcorrects" }
    ]
  });
  ...
  const expansionNaturalnessFamilies = [
    ...
    },
    {
      ...
      variants: [
        ["A research summary says the sample is small but the pattern is consistent. Choose the best conclusion.", "The pattern is worth exploring, but stronger evidence is needed.", "The result proves the theory.", "The sample is small, so the result is useless.", 0.74, 0.18, "weak", "overstates_or_dismisses"],
        ["A client says, 'It's closer to what we need, though not quite there yet.' Choose the best internal note.", "The revision is an improvement, but further work is needed.", "The client is satisfied.", "The client says it is close but not there.", 0.72, 0.42, "weak_nonnative", "literal_summary"],
        ["A manager says, 'Let's not rule it out, but it is not our priority.' Choose the best action.", "Keep the option open while focusing elsewhere.", "Start working on it immediately.", "Remove it from the plan.", 0.72, 0.14, "weak", "wrong_action"],
        ["A report states that the policy reduced costs at the expense of service quality. Choose the best summary.", "Costs fell, but service quality suffered.", "The policy was successful.", "Service quality and cost were both reduced well.", 0.78, 0.16, "weak", "misreads_tradeoff"],
        ["A colleague says, 'The argument is plausible, but the wording is too strong.' Choose the best revision goal.", "Keep the idea but soften the claim.", "Delete the argument.", "Make the wording stronger.", 0.76, 0.12, "weak", "reverses_goal"],
        ...
        ["A project note says, 'Speed should not come at the cost of reliability.' Choose the best priority.", "Reliability should be protected even if progress is slower.", "Speed is more important than reliability.", "Reliability and speed are the same.", 0.78, 0.1, "weak", "reverses_priority"]
      ].map(naturalnessVariant)
    }
  ];
